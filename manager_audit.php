...
// Get employee info & facility
$user_id = $_SESSION['user_id'];
$stmt = $conn->prepare(
    "SELECT e.first_name, e.last_name, e.role AS emp_role, e.facility_id,
            CONCAT(f.city,', ',f.state,' — ',f.type) AS facility_label
     FROM Employee e JOIN Facility f ON e.facility_id = f.facility_id
     WHERE e.user_id = ?"
);
...
if (!$emp || !in_array($emp['emp_role'], ['Manager','Admin'])) {
...
$facility_label = $emp['facility_label'];
...
// ── Filter option lists (scoped to this facility) ─────────────────────────────
$action_types = [];
$res = $conn->prepare("SELECT DISTINCT action_type FROM Audit_Log WHERE facility_id = ? ORDER BY action_type");
$res->bind_param("i", $facility_id);
$res->execute();
$r2 = $res->get_result();
while ($r = $r2->fetch_assoc()) $action_types[] = $r['action_type'];

$entity_types = [];
$res3 = $conn->prepare("SELECT DISTINCT entity_type FROM Audit_Log WHERE facility_id = ? ORDER BY entity_type");
$res3->bind_param("i", $facility_id);
$res3->execute();
$r4 = $res3->get_result();
while ($r = $r4->fetch_assoc()) $entity_types[] = $r['entity_type'];
...
$cs = $conn->prepare("SELECT COUNT(*) FROM Audit_Log a
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="facility_audit_' . date('Ymd_His') . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Log ID','Timestamp','Action','Entity','Entity ID','Performed By','Role','Description','IP']);
    foreach ($logs as $l) {
        fputcsv($out, [$l['log_id'],$l['created_at'],$l['action_type'],$l['entity_type'],
            $l['entity_id'],$l['performer_name'],$l['performer_role'],$l['description'],$l['ip_address']]);
    }
    fclose($out); exit();
}
...
$today  = date('Y-m-d');
$s_today = $conn->prepare("SELECT COUNT(*) FROM Audit_Log WHERE facility_id=? AND DATE(created_at)=?");
$s_today->bind_param("is", $facility_id, $today);
$s_today->execute();
$count_today = $s_today->get_result()->fetch_row()[0];

$s_pkg = $conn->prepare("SELECT COUNT(*) FROM Audit_Log WHERE facility_id=? AND entity_type='Package' AND DATE(created_at)=?");
$s_pkg->bind_param("is", $facility_id, $today);
$s_pkg->execute();
$count_pkg_today = $s_pkg->get_result()->fetch_row()[0];

$s_week = $conn->prepare("SELECT COUNT(*) FROM Audit_Log WHERE facility_id=? AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)");
$s_week->bind_param("i", $facility_id);
$s_week->execute();
$count_week = $s_week->get_result()->fetch_row()[0];
...
<head>
    ...
    <title>Facility Audit Log</title>
...
<!-- NAV -->
<nav class="bg-blue-900 text-white px-6 py-3 flex items-center justify-between shadow-lg sticky top-0 z-50">
    <div class="flex items-center gap-4">
        <a href="employee_dashboard.php" class="font-black text-lg tracking-tight hover:text-red-400 transition">POSTAL PRO</a>
        <span class="text-blue-300 text-sm hidden sm:block">/ Facility Audit Log</span>
    </div>
    <div class="flex items-center gap-3 text-sm">
        <a href="employee_dashboard.php" class="hover:text-blue-200 transition"><i class="fas fa-arrow-left mr-1"></i>Dashboard</a>
        <a href="logout.php" class="bg-red-600 hover:bg-red-700 px-3 py-1 rounded text-white transition"><i class="fas fa-sign-out-alt mr-1"></i>Logout</a>
    </div>
</nav>
...
<div class="max-w-screen-xl mx-auto px-4 py-8">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-black text-gray-900">Facility Audit Log</h1>
            <p class="text-gray-500 text-sm mt-1">
                <i class="fas fa-map-marker-alt mr-1 text-blue-600"></i>
                <?= htmlspecialchars($facility_label) ?>
                &mdash; <?= number_format($total_rows) ?> records matching current filters
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            <div class="w-11 h-11 rounded-xl bg-blue-100 flex items-center justify-center text-blue-700 text-lg">
                <i class="fas fa-calendar-day"></i>
            </div>
            <div>
                <div class="text-2xl font-black text-gray-900"><?= number_format($count_today) ?></div>
                <div class="text-xs text-gray-500 font-semibold uppercase tracking-wide">Events Today</div>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            <div class="w-11 h-11 rounded-xl bg-indigo-100 flex items-center justify-center text-indigo-700 text-lg">
                <i class="fas fa-box"></i>
            </div>
            <div>
                <div class="text-2xl font-black text-gray-900"><?= number_format($count_pkg_today) ?></div>
                <div class="text-xs text-gray-500 font-semibold uppercase tracking-wide">Package Events Today</div>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            <div class="w-11 h-11 rounded-xl bg-purple-100 flex items-center justify-center text-purple-700 text-lg">
                <i class="fas fa-calendar-week"></i>
            </div>
            <div>
                <div class="text-2xl font-black text-gray-900"><?= number_format($count_week) ?></div>
                <div class="text-xs text-gray-500 font-semibold uppercase tracking-wide">Events This Week</div>
            </div>
        </div>
    </div>
    ...
    <form method="get" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">Performed By</label>
        ...
        <div class="mt-3 flex justify-between items-center">
            <a href="manager_audit.php" class="text-sm text-gray-400 hover:text-gray-600 transition">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="text-center py-16 text-gray-400">
            ...
            <p class="font-semibold">No activity recorded yet for this facility.</p>
            <p class="text-sm mt-1">Events will appear here as your team processes packages, scans, and trips.</p>
        </div>
        <?php else: ?>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100 text-xs text-gray-500 uppercase tracking-wide">
                    ...
                    <th class="text-center px-4 py-3 w-8">Detail</th>
                </tr>
            </thead>
            ...
            <?php foreach ($logs as $l): ?>
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-4 py-3 text-gray-600 max-w-sm">
                        <?= htmlspecialchars(mb_strimwidth($l['description'], 0, 90, '…')) ?>
                    </td>
                ...
                <tr id="<?= $row_id ?>-detail" class="hidden bg-gray-50 border-b border-gray-100">
                    <td colspan="7" class="px-6 py-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <?php if ($l['old_value']): ?>
                            <div>
                                <p class="text-xs font-bold text-red-500 uppercase mb-1"><i class="fas fa-minus-circle mr-1"></i>Before</p>
                            ...
                            <?php if ($l['new_value']): ?>
                            <div>
                                <p class="text-xs font-bold text-green-600 uppercase mb-1"><i class="fas fa-plus-circle mr-1"></i>After</p>
        ...
        <div class="px-4 py-4 border-t border-gray-100 flex flex-wrap items-center justify-between gap-3">
            <span class="text-sm text-gray-500">
                Showing <?= number_format($offset+1) ?>–<?= number_format(min($offset+$per_page, $total_rows)) ?> of <?= number_format($total_rows) ?>
            </span>
