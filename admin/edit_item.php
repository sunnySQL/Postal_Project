...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($description) || empty($category) || $price_wholesale <= 0 || $sale_price <= 0) {
        ...
        if (empty($error)) {
            ...
            try {
                $stmt = $conn->prepare("UPDATE Items SET name = ?, price_wholesale = ?, sale_price = ?, 
                                     description = ?, category = ?, image = ? WHERE item_id = ?");
                $stmt->bind_param("sddsssi", $name, $price_wholesale, $sale_price, $description, $category, $image_path, $item_id);
                
                if ($stmt->execute()) {
                    $_SESSION['message'] = "Item #$item_id updated successfully.";
                    $_SESSION['message_type'] = "success";
                    logAudit($conn, 'ITEM_UPDATED', 'Inventory', (string)$item_id,
                        "Item #{$item_id} \"{$name}\" updated",
                        null,
                        ['name' => $item['name'], 'category' => $item['category'],
                         'sale_price' => $item['sale_price'], 'price_wholesale' => $item['price_wholesale']],
...
<div class="max-w-2xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-black text-gray-900">Edit Item <span class="text-[#004B87]">#<?= $item_id ?></span></h1>
            <p class="text-sm text-gray-500 mt-0.5"><?= htmlspecialchars($item['name'] ?? '') ?> &mdash; <?= htmlspecialchars($item['category'] ?? '') ?></p>
        </div>
        ...
        <form action="edit_item.php?id=<?= $item_id ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="category" id="category-select"
                    ...
                    <option value="<?= htmlspecialchars($cat) ?>"
                            <?= ((isset($_POST['category']) && $_POST['category'] === $cat) || (!isset($_POST['category']) && $item['category'] === $cat)) ? 'selected' : '' ?>>
                ...
                <input type="text" name="category" id="category-input"
                       ...
                       value="<?= htmlspecialchars($_POST['category'] ?? $item['category'] ?? '') ?>">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="price_wholesale" step="0.01" min="0.01" required
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#004B87]"
                           value="<?= htmlspecialchars($_POST['price_wholesale'] ?? $item['price_wholesale'] ?? '') ?>">
                </div>
                <div>
                    ...
                    <input type="number" name="sale_price" step="0.01" min="0.01" required
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#004B87]"
                           value="<?= htmlspecialchars($_POST['sale_price'] ?? $item['sale_price'] ?? '') ?>">
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3" required
                          class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#004B87]"><?= htmlspecialchars($_POST['description'] ?? $item['description'] ?? '') ?></textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="mb-3 flex items-center gap-3">
                    <img src="../<?= htmlspecialchars($item['image']) ?>" alt="Current image" class="h-16 w-16 object-cover rounded-lg border border-gray-100">
                    <p class="text-xs text-gray-400">Current image &mdash; upload a new one to replace.</p>
                </div>
                <?php endif; ?>
                ...
                <p class="text-xs text-gray-400 mt-1">Leave empty to keep current image. JPG, PNG or GIF. Max 5MB.</p>
            </div>
